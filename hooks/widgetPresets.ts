...
export const ANALOG_FACE_PRESETS: StylePreset[] = [
  { key: 'classic', label: '经典' },   // 刻度线 + 指针 + 内外双环
  { key: 'minimal', label: '极简' },   // 无刻度、无表圈，纯指针
  { key: 'dots',    label: '点状' },   // 圆点刻度
  { key: 'modern',  label: '现代' },   // 双进度弧 + 中央数字
  { key: 'swiss',   label: '瑞士' },   // 瑞士火车站风格
  { key: 'roman',   label: '罗马' },   // 罗马数字 + 复古质感
];

// ── 日历 布局风格 ─────────────────────────────────────────────────────────

export const CALENDAR_STYLE_PRESETS: StylePreset[] = [
  { key: 'text',     label: '文本' },   // 一行文字 + 上下细线装饰
  { key: 'badge',    label: '徽章' },   // 大日期数字 + 玻璃质感
  { key: 'card',     label: '卡片' },   // 大号日期卡片 + 顶部装饰
  { key: 'stamp',    label: '邮票' },   // 双层框 + 四角装饰圆点
  { key: 'expanded', label: '详细' },   // 左大数字 + 右元数据列
];
